...
interface UserProfile {
  ...
  /** Server-computed: does deleting/re-auth require entering a password? */
  hasPassword?: boolean;
}
...
export default function SettingsPage({ section, isMember = false }: { section: TabId; isMember?: boolean }) {
  ...
  useEffect(() => {
    ...
    // One-shot params: strip success/error/payment from the address bar so
    // refreshing never re-fires the notification (and the #_ OAuth fragment
    // goes with it). history.replaceState = no reload, no new history entry.
    if (paymentStatus || searchParamsMain.get("success") || searchParamsMain.get("error")) {
          ...
          <div className="mt-10 pt-6 border-t border-red-200 dark:border-red-900/50">
            ...
            {!showDeleteConfirm ? (
              ...
              <div className="space-y-3 p-4 rounded-xl border border-red-300 dark:border-red-800 bg-red-50 dark:bg-red-950/20">
                ...
                {/* Step 2: accounts with a password re-enter it; Google-only
                    accounts see a final warning instead. Driven by the same
                    server-computed flag the delete action uses. */}
                {profile?.hasPassword ? (
                  <>
                    <p className="text-sm text-red-700 dark:text-red-400 font-medium">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    onClick={async () => {
                      setDeleting(true);
                      setDeleteError("");
                      // Google sign-ups have no password; the action decides
                      // whether one is required from the session itself.
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="rounded-xl border border-border bg-card p-5">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Top-up +500 DMs for ₹99 — no plan change.
          </p>
          <button
                    ...
                    if (w) {
                      w.document.write(`<!DOCTYPE html><html><head><title>Invoice ${invNo}</title><style>
                        *{margin:0;padding:0;box-sizing:border-box}
                        ...
                        <div class="parties">
                          <div class="party"><h4>From</h4><p><strong>NovaMint Networks</strong><br/>${process.env.NEXT_PUBLIC_SELLER_ADDRESS || "India"}<br/>chirplymint.novamintnetworks.in · ChirplyMint@gmail.com</p></div>
